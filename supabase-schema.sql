...
-- ══ INVENTARIO (RPCs) ══
-- La app gestiona el inventario con estas RPCs, que ahora también registran
-- inventory_movements (scripts/fix-inventory-movements.sql). Los parámetros de
-- contexto (movement_type, reference_type, reference_id) son opcionales.

-- Helper de diagnóstico: lista funciones del schema public (evita falsos negativos)
