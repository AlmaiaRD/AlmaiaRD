...
const TABLES = [
  ...
  "bonuses", "settings", "audit_logs", "communications", "returns",
  "return_items", "bundle_items", "whatsapp_configs", "whatsapp_logs",
];
...
for (const t of TABLES) {
  ...
  try {
    const res = await supabase.from(t).select("*", { count: "exact" });
    data = res.data;
    error = res.error;
  } catch (e) {
